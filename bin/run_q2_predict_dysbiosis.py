    ...
    if mode == "no_taxonomy":
        ...
        for sample in sample_list:
            ...
            try:
                ...
                # Handle case where no pathways are found (all zeros)
                pathways_with_values = len(sample_paths_unstrat[sample_paths_unstrat > 0].index)
                if pathways_with_values > 0:
                    core_functions_fraction_in_all = core_found/pathways_with_values
                else:
                    core_functions_fraction_in_all = 0
...
def main():
    parser = argparse.ArgumentParser(description='Calculate dysbiosis scores from microbiome data')
    
    # Required arguments
    parser.add_argument('--species', '-s', required=True, 
                        help='Path to species abundance file (tab-separated)')
    parser.add_argument('--pathways-stratified', '-ps', required=True,
                        help='Path to stratified pathways file (tab-separated)')
    parser.add_argument('--pathways-unstratified', '-pu', required=True,
                        help='Path to unstratified pathways file (tab-separated)')
    
    # Optional arguments
    parser.add_argument('--mode', '-m', choices=['full', 'no_taxonomy'], default='full',
                        help='Analysis mode: "full" includes taxonomic features, "no_taxonomy" excludes them (default: full)')
    parser.add_argument('--output', '-o', default='dysbiosis_scores_output.csv',
                        help='Output CSV file name (default: dysbiosis_scores_output.csv)')
    ...
    for file_path, file_type in [(args.species, 'species'), 
                                 (args.pathways_stratified, 'pathways-stratified'),
                                 (args.pathways_unstratified, 'pathways-unstratified')]:
        if not os.path.exists(file_path):
            print(f"Error: {file_type} file '{file_path}' not found.", file=sys.stderr)
            sys.exit(1)
    ...
    try:
        ...
        taxa = pd.read_csv(args.species, sep="\t", index_col=0)
        paths_strat = pd.read_csv(args.pathways_stratified, sep="\t", index_col=0)
        paths_unstrat = pd.read_csv(args.pathways_unstratified, sep="\t", index_col=0)
        
        if not args.quiet:
            print(f"Loaded {taxa.shape[0]} species, {paths_strat.shape[0]} stratified pathways, {paths_unstrat.shape[0]} unstratified pathways")
            print(f"Analysis mode: {args.mode}")
        
        # Calculate dysbiosis scores
        if not args.quiet:
            print("Calculating dysbiosis scores...")
            
        results = calculate_index(taxa, paths_strat, paths_unstrat, args.mode)
